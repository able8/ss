...
try:
    ...
    if os.path.exists(user_config_json_file):
        ...
        if user_config['apache_http_auth'] != {}:
            http_auth_user = user_config['apache_http_auth'][user_config['apache_http_auth'].keys()[0]]
            http_auth_pass = user_config['apache_http_auth'].keys()[0]
        else:
            http_auth_user = "gfw"
...
new_cipher_json = json.read_json_from_file_return_dic("%s/ciphers.py" % tmp_dir)
if os.path.exists("/data/ss_client/shadowsocks/crypto/ciphers.py") is not True:
    rewrite_file = "yes"
else:
    ori_ciphers = funcs.read_json_from_file_return_dic("/data/ss_client/shadowsocks/crypto/ciphers.py")
...
if str(before_json_dic['method']).strip() != str(new_config_json['method']).strip():
    ...
    rewrite_file = "yes"
if compare_two_dic(before_config_json['port_password'],new_config_json['port_password']) == "yes":
    rewrite_file = "yes"
#if change_port == "yes":
...
if rewrite_file == "yes":
    before_config_json['port_password'] = new_config_json['port_password']
