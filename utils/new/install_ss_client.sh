...
bash /data/ss_client/utils/polipo.sh "0.0.0.0" "18888" "127.0.0.1" "7790" > /data/logs/ss_client/polipo.log 2>&1 &

info_me "deal with modify holy.sh"
if [ "$domain" != "no_change" ];then
    sed -i "s/DOMAIN/${domain}/" /data/ss_client/utils/holy.sh
fi
sed -i "s/PORT/${apache_port}/" /data/ss_client/utils/holy.sh
...
* * * * * root bash /data/ss_client/utils/si_2.sh /data/ss_client/utils/new/check_socks_local.sh
